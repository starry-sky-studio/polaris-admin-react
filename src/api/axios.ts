...
class HttpRequest {
  ...
  public constructor() {
    ...
    this.instance.interceptors.response.use(
      ...
      async (err: AxiosError<R>) => {
         ...
         * - 其他状态码，提示错误信息
         */
        const errorMessage = msg ?? errorMessageMap.get(status as number) ?? '未知的错误'
        switch (status) {
          case StatusCode.UNAUTHORIZED:
            // 存在刷新令牌，认证令牌过期时，需要通过刷新令牌获取新的认证令牌
            await this.handleRefreshToken(config, errorMessage)
            break
          case StatusCode.FORBIDDEN:
            message.error(errorMessage)
            router.navigate('/error-pages/403', { replace: true })
            break
          case StatusCode.INTERNAL_SERVER_ERROR:
          case StatusCode.BAD_GATEWAY:
          case StatusCode.GATEWAY_TIMEOUT:
            message.error(errorMessage)
            router.navigate('/error-pages/500', { replace: true })
            break
          default:
            message.error(errorMessage)
            break
  ...
  async handleRefreshToken(
    ...
    if (currentRefreshToken) {
      ...
      try {
        const { refreshToken, accessToken } = (await AuthAPI.refresh(currentRefreshToken)).data!
